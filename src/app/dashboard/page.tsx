...
import { Flame, TrendingUp, Sparkles, Video, Lock, Unlock, Map as MapIcon, Gem, ChevronRight, X, PlayCircle } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  return (
    <main className="min-h-full pb-24 bg-slate-50 flex flex-col relative">
      <header className="px-6 pt-10 pb-8 bg-white rounded-b-3xl shadow-sm border-b border-slate-100 z-10 relative overflow-hidden mb-8">
      ...
      <div className="flex-1 bg-slate-50 space-y-10">
        {/* 1. 요새 뜨는 직업 (순위 표기) */}
        <section className="px-6">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar snap-x px-2 -mx-2">
            {TRENDING_JOBS.map((job, idx) => (
              <div 
                key={idx} 
                onClick={() => setSelectedJob(getJobInfo(job))}
                className="bg-white hover:bg-slate-50 border border-slate-100 transition-colors rounded-2xl p-6 min-w-[160px] snap-center flex-shrink-0 cursor-pointer relative shadow-sm"
              >
                <div className="absolute -top-3 -left-3 w-8 h-8 bg-primary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm">
                  {idx + 1}
                </div>
                <TrendingUp className="w-8 h-8 text-primary mb-3 mt-1" />
          ...
          <div className="space-y-3">
            {activeCategory === 'recommended' && recommendedJobs.map((job: string) => (
              <div 
                key={job} 
                onClick={() => setSelectedJob(getJobInfo(job))}
                className="w-full bg-white hover:bg-slate-50 border border-slate-100 rounded-2xl p-5 flex justify-between items-center cursor-pointer transition-all shadow-sm active:scale-[0.98]"
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center text-primary">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <span className="text-slate-900 font-bold text-base">{job}</span>
                </div>
        ...
        {/* 3. 리얼 직무 VLOG 섹션 */}
        <section className="px-6 space-y-10 pb-8">
          <div>
            <h2 className="text-slate-900 font-extrabold text-xl mb-5 flex items-center gap-2">
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 snap-x hide-scrollbar px-2 -mx-2">
              {vlogs.map((vlog: any) => (
                <div 
                  ...
                  className="min-w-[260px] h-40 bg-white rounded-3xl relative overflow-hidden flex-shrink-0 snap-center cursor-pointer group border border-slate-100 shadow-sm flex items-center justify-center"
                >
                  {/* 썸네일 이미지 렌더링 */}
                  {vlog.thumbnailUrl ? (
                  ...
                  {(!vlog.videoUrl) ? (
                    <div className="absolute inset-0 bg-white/40 backdrop-blur-[4px] flex flex-col items-center justify-center group-hover:bg-white/50 transition-colors z-10">
                      <div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center mb-2 text-slate-800">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center z-10 opacity-0 group-hover:opacity-100 transition-opacity bg-black/20 backdrop-blur-sm">
                      <div className="w-14 h-14 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-primary pl-1 shadow-sm">
            ...
            <div className="relative bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
              <div className={`p-6 ${!isSubscribed ? 'h-56 filter blur-[8px] opacity-40 overflow-hidden' : ''}`}>
                ...
                {!isSubscribed ? (
                  ...
                  <div className="space-y-6 relative before:absolute before:inset-0 before:ml-4 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-slate-100">
              ...
              {!isSubscribed && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-50/50 backdrop-blur-[2px]">
                  ...
                  >
                    <span className="bg-slate-100 w-8 h-8 rounded-full flex items-center justify-center text-sm text-slate-800"><Unlock className="w-4 h-4" /></span> 프리미엄 멤버십으로 열람하기
                  </button>
                </div>
              )}
            </div>
      ...
      {showPremiumModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-sm p-8 relative shadow-2xl animate-in fade-in zoom-in duration-200 border border-slate-100">
            <button onClick={() => setShowPremiumModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 bg-slate-50 w-8 h-8 rounded-full flex items-center justify-center"><X className="w-5 h-5" /></button>
            <div className="text-center mt-2">
              ...
              <button 
                ...
                className="w-full py-4 bg-gradient-to-r from-primary to-indigo-500 text-white font-extrabold text-lg rounded-2xl shadow-sm active:scale-95 transition-transform"
              >
                지금 바로 구독하기
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-900/40 backdrop-blur-sm sm:items-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white sm:border border-slate-100 rounded-t-[2rem] sm:rounded-3xl w-full max-w-[480px] sm:max-w-sm max-h-[85vh] overflow-y-auto relative shadow-2xl animate-in slide-in-from-bottom duration-300">
            <div className="sticky top-0 bg-white/90 backdrop-blur-md pb-4 pt-8 px-8 border-b border-slate-100 z-10 flex justify-between items-center">
              ...
              <button onClick={() => setSelectedJob(null)} className="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 hover:text-slate-900"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-8">
              <ul className="space-y-6 text-sm">
                <li>
                  ...
                  <span className="text-slate-700 font-medium leading-relaxed bg-slate-50 p-4 rounded-2xl block border border-slate-100">{selectedJob.tasks}</span>
                </li>
                <li>
                ...
                <li>
                  ...
                  <span className="text-slate-700 font-medium leading-relaxed block bg-slate-50 p-4 rounded-2xl border border-slate-100">{selectedJob.certs}</span>
                </li>
                <li>
                  <span className="block text-slate-400 mb-2 text-xs font-bold uppercase tracking-wider">주요 채용 회사</span>
                  <span className="text-slate-700 font-medium block bg-slate-50 p-4 rounded-2xl border border-slate-100">{selectedJob.companies}</span>
                </li>
                <li>
                  <span className="block text-slate-400 mb-2 text-xs font-bold uppercase tracking-wider">직무 만족도</span>
                  <span className="text-yellow-500 font-extrabold text-lg block bg-slate-50 p-4 rounded-2xl border border-slate-100">⭐️ {selectedJob.satisfaction}</span>
                </li>
              </ul>
